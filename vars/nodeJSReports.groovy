    ...
    node(label) {
      notifyStatus {
        ...
        container('node') {
          ...
          // container('docker') {
          //   stage('Run Docker Datastore') {
          //     docker.image('singularities/datastore-emulator:latest').withRun('--name datastore -e "DATASTORE_PROJECT_ID=test" -e "DATASTORE_LISTEN_ADDRESS=0.0.0.0:8081" -p 8081:8081') {
          //     }
          //   }
          // }

          // container('docker') {
          //   stage('Run Docker Pubsub') {
          //     docker.image('singularities/pubsub-emulator:latest').withRun('--name datastore -e "PUBSUB_PROJECT_ID=test" -e "PUBSUB_LISTEN_ADDRESS=0.0.0.0:8432" -p 8432:8432') {
          //     }
          //   }
          // }
