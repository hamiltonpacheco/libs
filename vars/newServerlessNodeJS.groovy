def call(Map args) {
  ...
  def credentialsId = credentials.id ?: null
  def credentialsPath = credentials.path ?: null
        ...
        gitlabCommitStatus(name: currentBuild.fullDisplayName) {
          ...
          if (masterBranch) {
            ...
            stage('Deploy Serverless') {
              container('node') {
                if (!credentialsId || !credentialsPath) {
                  
                  def prodEnvList = buildEnvVarList(productionEnv)
                  withEnv(prodEnvList) {
                    sh 'npm run deploy'
                  }
                  ...
                  withCredentials([ file(credentialsId: credentialsId, variable: "CREDENTIALS_FILE") ]) {
                    def credentialsAbsolutePath = "${WORKSPACE}/${credentialsPath}"
                    echo "creating credentials file at ${credentialsAbsolutePath}"
                    sh(script: 'cat $CREDENTIALS_FILE > ' + credentialsAbsolutePath)
                    
                    def prodEnvList = buildEnvVarList( productionEnv << ["WORKSPACE": "${WORKSPACE}"] )
